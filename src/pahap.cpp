...
{
  ROS_INFO("Pahap Constructor, name:%s", t_Name.c_str());
  pclTopic_ = t_Name;
...
// advertise topic "processed_pcl" with message type sensor_msgs::PointCloud2
// this broadcasts a point cloud message on a topic called processed_pcl
...
bool PahaP::boundaryEstimate(pahap::pcl_boundary::Request &req,
  ...
  if (req.cmd) {
    ...
    for (int j=0; j< req.clusterSet.size(); j++) {
      ...
      bound = est_2DBound(ppoints, max_min_xy, 0.03); // slicing_factor =0.02
      int count =0;
...
{
  ROS_INFO("Pahap main function starting");
  ...
  std::string filename;
  n_para.param("fileAddress", filename, std::string("/home/alw/ara_navigation/src/pahap/src/L_shape.pcd"));
  ROS_INFO("Starting PahaP construction, topic name: %s", pclTopic.c_str());
